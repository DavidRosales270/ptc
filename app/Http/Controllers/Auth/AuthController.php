    ...
    */
	...
	public function confirmRegister($email, $confirm_token)
	{
		...
		if(count($the_user) > 0)
		{
			...
			->with('message', 'En hora buena '.$the_user[0]['name'].' ya puede iniciar sesión');
		}
		else
		{
			return redirect('auth/register')
			->with('message', 'No es posible activar al usuario '.$the_user[0]['name']);
		}
	}
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$_POST['do'] == 'contact') 
	{
		...
		$captcha = @$_POST['ct_captcha']; // the user's entry for the captcha code
	  	...
	  	if (sizeof($errors) == 0) {
	  		//include("public/securimage/securimage.php");
		    require_once 'public/securimage/securimage.php';
		    $securimage = new Securimage();
				
			if ($securimage->check($captcha) == false) {
				$errors['captcha_error'] = 'Código captcha incorrecto';
			}
		}
	}
	...
	if (sizeof($errors) == 0) {
	        ...
	        $messages = [
	            ...
	            'name.unique' => 'Ya existe un usuario registrado con el mismo nombre',
	            'email.required' => 'El campo es requerido',
	            'email.email' => 'El formato de email es incorrecto',
	            'email.max' => 'El máximo de caracteres permitidos son 255',
	            'email.unique' => 'Ya existe un usuario registrado con el mismo Email',
	            'ip.required' => 'No se pudo obtener información de la ip pública',
	            'ip.unique' => 'Sólo es permitido el registro de un usuario por ip pública', 
	            'password.required' => 'El campo es requerido',
	            'password.min' => 'El mínimo de caracteres permitidos son 6',
	            'password.max' => 'El máximo de caracteres permitidos son 18',
	            'password.confirmed' => 'Los passwords no coinciden',
	            'anionac.required' => 'El campo es requerido',
	            'anionac.min' => 'El mínimo de caracteres permitidos son 4',
	            'anionac.max' => 'El máximo de caracteres permitidos son 4',
	        ];
        
	        //$arrx = $request->all();
	        $ip = $request->ip();
	        $arrx = array_merge($request->all(), ['ip' => $ip]);
	        
	        $validator = Validator::make($arrx, $rules, $messages);
	        ...
	        if ($validator->fails()){
	            ...
	            $user->referente = $request->referente;
	            $user->anionac = $request->anionac;
	            $user->ip = $request->ip();
  		...
  		return redirect('auth/register')           
	          	->withInput()
	            	->with('captcha', $errors['captcha_error']);
	}        
    }
		...
		$captcha = @$_POST['ct_captcha']; // the user's entry for the captcha code
  		 ...
  		 if (sizeof($errors) == 0) {
            ...
            $messages = [
                ...
                //'write.required' => 'El campo es requerido', 
            	//'write.confirmed' => 'Los códigos de confirmación no coinciden',
            ];
